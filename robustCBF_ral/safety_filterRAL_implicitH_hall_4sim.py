...
def get_h0(h, x, y):
    ...
    imax = 200.0 # Grid I Dimension (Negative Y-Direction)
...
class SSF:
    ...
    def __init__(
        ...
        self.c = 2.8 * np.pi / 6 # frequency parameter for the narrow passage
    ...
    def control_ssf(self, state, u_nominal):
        ...
        # --- derivatives that do NOT depend on γ -----------------
        h0, dhdx, dhdy, *_ = get_h0(self.hvalues, x_nom[0], x_nom[1])
        grad_h0   = np.array([dhdx, dhdy])
        Lf_h      = 0.0
        Lg_v      = grad_h0 @ np.array([np.cos(x_nom[2]),
                                        np.sin(x_nom[2])])
        Lg_vec    = np.array([Lg_v, 0.0])              # ω has no effect
        g_norm    = np.linalg.norm(Lg_vec) + 1e-12     # avoid /0
        alpha_hs        = self.alpha * h0

        # --- nominal command (time-varying!) ----------------------
        ...
        # ---------- safe control for the *nominal* state ----------
        ...
        self.k_1 = 0 * float(gammas1_flat[best])
        self.k_2 = 0 * float(gammas2_flat[best])
        ...
        # # record \dot{h} values
        # dh1 = (self.conspar.value * self.nominal_linear_acc.value)fdel
