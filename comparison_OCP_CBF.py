...
if robust_logs:
    ...
    ax_g.plot(cbf_t, cbf_g1, 'r', label=r'$\gamma_1(\mathbf{\hat{x}}$)')
    ax_g.plot(cbf_t, cbf_g2, 'g', label=r'$\gamma_2(\mathbf{\hat{x}})$')
...
if nominal_logs:
    ...
    ax_top.plot(cbf_xy[:,0], cbf_xy[:,1], 'k', lw=2, label=r'{R-CBF-QP (${\gamma_{1,2} \!=\!0}$)}', zorder=4)
    ...
    unique = {l: h for h, l in zip(handles, labels)}
    unique[r'{obstacle}'] = Line2D([0], [0], linestyle='None', marker='o', markersize=9, markeredgewidth=2, markeredgecolor='red', markerfacecolor='none')
    order = [r'{safe set}', r'{R-CBF-QP (${\gamma_{1,2} \!=\!0}$)}', r'{boundary}', r'{COCP}',  r'{obstacle}', r'{CBF-QP}',  r'{reference}', r'{$\mathbf{k_d}$}']
    order = [o for o in order if o in unique]
    ...
    ax_v.plot(cbf_t_n[:cbf_uv_n.shape[0]], cbf_uv_n[:, 0], 'k', label='R-CBF-QP (${\gamma_{1,2} \!=\!0}$)')
    ...
    ax_w.plot(cbf_t_n[:cbf_uv_n.shape[0]], cbf_uv_n[:, 1], 'k', label='R-CBF-QP (${\gamma_{1,2} \!=\!0}$)')
    if cbf2_uv is not None:
    ...
    ax_h.plot(cbf_t_n, cbf_h, 'k', label=r'R-CBF-QP (${\gamma_{1,2} \!=\!0}$)')
    ...
    ax_g.plot(cbf_t_n, np.round(cbf_g1, 1), 'r', label=r'$\gamma_1$')
    ax_g.plot(cbf_t_n, np.round(cbf_g2, 1), 'g', linestyle='--', label=r'$\gamma_2$')
    ax_g.set_ylabel(r'$\gamma$'); ax_g.set_xlabel(r'$t~[\text{s}]$')
...
# # ===========================================================
# t_kd = np.arange(U_kd.shape[0]) * dt

# kd_fun = interp1d(t_kd, U_kd, axis=0, kind='linear', fill_value='extrapolate', assume_sorted=True)

# def cost_integral_wrt_kd(t_arr, u_arr):
#     kd_at_t = kd_fun(t_arr)
#     diff_sq = np.einsum('ij,ij->i', u_arr, u_arr) \
#             + np.einsum('ij,ij->i', kd_at_t, kd_at_t) \
#             - 2*np.einsum('ij,ij->i', u_arr, kd_at_t)
#     return np.trapz(diff_sq, t_arr)

# costs = {}

# # OCP (robust)
# costs['OCP (robust)'] = cost_integral_wrt_kd(t_u, U_opt)
# if 'U_opt_nom' in globals():        costs['OCP (nom)']  = cost_integral_wrt_kd(t_u_nom, U_opt_nom)

# # Robust CBFs
# if 'cbf_uv' in globals():   costs['CBF (robust)']  = cost_integral_wrt_kd(cbf_t, cbf_uv)
# costs['CBF (nom)']          = cost_integral_wrt_kd(cbf_t_n, cbf_uv_n)

# if 'fx_uv'   in globals():   costs['Fixed-CBF']     = cost_integral_wrt_kd(fx_t,  fx_uv)
# if 'tn_uv'   in globals():   costs['Tunable-CBF']   = cost_integral_wrt_kd(tn_t,  tn_uv)
# if 'cbf2_uv' in globals():   costs['CBF w/o DRD']   = cost_integral_wrt_kd(cbf2_t, cbf2_uv)

# print("\n=============")
# for k, v in costs.items():
#     print(f"{k:<15s}: {v:9.4f}")
# print("==============\n")
...
# series = []                           
# series.append((t_kd, U_kd, r'$k_d$'))            

# # if 'U_opt'      in globals(): series.append((t_u,       U_opt,       'OCP (robust)'))
# # if 'U_opt_nom'  in globals(): series.append((t_u_nom,   U_opt_nom,   'OCP (nom)'))

# # if 'cbf_uv'     in globals(): series.append((cbf_t,     cbf_uv,      'CBF (robust)'))
# if 'cbf_uv_n'   in globals(): series.append((cbf_t_n,   cbf_uv_n,    'CBF (nom)'))

# # if 'fx_uv'      in globals(): series.append((fx_t,      fx_uv,       'Fixed CBF'))
# # if 'tn_uv'      in globals(): series.append((tn_t,      tn_uv,       'Tunable CBF'))
# # if 'cbf2_uv'    in globals(): series.append((cbf2_t,    cbf2_uv,     'CBF w/o DRD'))

# def plot_control(component, ylabel, filename):
#     plt.figure(figsize=(8, 3.2))
#     color_cycle = cycle(plt.rcParams['axes.prop_cycle'].by_key()['color'])
#     for t, u, lab in series:
#         c = next(color_cycle)
#         plt.plot(t, u[:, component], label=lab, color=c, linewidth=2)
#     plt.xlabel(r'$t$, [s]')
#     plt.ylabel(ylabel)
...
# plot_control(0, r'$v\;[\mathrm{m/s}]$',        'v_all_methods.pdf')
# plot_control(1, r'$\omega\;[\mathrm{rad/s}]$', 'w_all_methods.pdf')

# # ---- decompose J into v and omega parts for any controller 
# def split_cost(t, u):
#     kd = kd_fun(t)
#     dv2 = (u[:,0] - kd[:,0])**2
#     dw2 = (u[:,1] - kd[:,1])**2
#     return np.trapz(dv2, t), np.trapz(dw2, t)

# for lab, (t, u) in [('CBF (nom)', (cbf_t_n, cbf_uv_n))]:
#     Jv, Jw = split_cost(t, u)
#     print(f'{lab:15s}:  v-part = {Jv:7.3f}   omega-part = {Jw:7.3f}   total = {Jv+Jw:7.3f}')


# -------------------------------------------------------------------
#  2-norm tracking cost  J(x,u, t)
...
# def J_track(t, x, u, baseline='robust'):
#     if baseline == 'robust':
#         du = u - u_opt_fun_rob(t)
#         dx = x - x_opt_fun_rob(t)
#     else:  # 'nominal'
#         du = u - u_opt_fun_nom(t)
#         dx = x - x_opt_fun_nom(t)

#     term_u = np.einsum('ij,jk,ik->i', du, Q_u, du)      
#     term_x = np.einsum('ij,jk,ik->i', dx, R_x, dx)      
#     return float(np.trapz(term_u + term_x, t))
...
# state+input arrays for each controller
...
if robust_logs:
    ...
    # we can compute the baseline itself (=0)
