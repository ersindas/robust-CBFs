...
obs   = [ 3.0 0.0 0.5 ;      % [x y R] obstacle-1
              6.0 0.0 0.5 ];     % obstacle-2

% data
bag = rosbag('robot_control_2025-02-28-19-38-06.bag');
bagInfo = rosbag('info','robot_control_2025-02-28-19-38-06.bag');
bag_p = rosbag('robot_state_2025-02-28-19-38-05.bag');
bagInfo_p = rosbag('info','robot_state_2025-02-28-19-38-05.bag');
...
h2_value = cellfun(@(m) double(m.H2),msgStructs_des);
h3_value = cellfun(@(m) double(m.H3),msgStructs_des);
...
b = 0;
a = 0.0;
d = 3.03/2 - 0.4; % safety boundary radius
h_values = d^2 - y_p.^2;
...
plot(t', h2_value, 'r', 'DisplayName', 'CBF $h_{obs1}(x, y)$');
plot(t', h3_value, 'b', 'DisplayName', 'CBF $h_{obs2}(x, y)$');
xlabel('$t \ [s]$');
...
y_safe_upper = 0 * sin(c * x_safe + b) + a + d;
y_safe_lower = 0 * sin(c * x_safe + b) + a - d;
...
y_safe_upper_d1 = 0 * sin(c * x_safe + b) + a + d;
y_safe_lower_d1 = 0 * sin(c * x_safe + b) + a - d;
